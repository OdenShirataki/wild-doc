...
pub(super) fn case<T:IncludeAdaptor>(script:&mut Script,e:&BytesStart,xml_str:&str,worker: &mut MainWorker,include_adaptor:&mut T)->Result<Vec<u8>,std::io::Error>{
    let mut r=Vec::new();
                                ...
                                match next{
                                    Event::Start(ref e)=>{
                                        match e.name().as_ref(){
                                            b"wd:when"=>{
                                                let attr=xml_util::attr2hash_map(&e);
                                                let wv=crate::attr_parse_or_static(worker,&attr,"value");
                                                if wv==cmp_value{
                                                        ...
                                                        match event_reader_inner.read_event(){
                                                            Ok(Event::Start(e))=>{
                                                                if e.name().as_ref()==b"wd:when"{
                                                                    r.append(&mut script.parse(worker,&mut event_reader_inner,"",include_adaptor)?);
                                                                    break 'case;
                                                                }
                                                            }
                                                            ,_=>{}
...
pub(super) fn r#for<T:IncludeAdaptor>(script:&mut Script,e:&BytesStart,xml_str:&str,worker: &mut MainWorker,include_adaptor:&mut T)->Result<Vec<u8>,std::io::Error>{
    let mut r=Vec::new();
    let attr=xml_util::attr2hash_map(&e);
                            ...
                            match ev.read_event(){
                                Ok(Event::Start(e))=>{
                                    if e.name().as_ref()==b"wd:for"{
                                        ...
                                        r.append(&mut script.parse(worker,&mut ev,"wd:for",include_adaptor)?);
